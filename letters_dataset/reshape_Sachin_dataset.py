...
with open(file_path, newline='') as csvfile:
    ...
    for row in reader:
        ...
        count += 1
    # Append count of last letter
    letter_count.append(count)
# [1:] to eliminate first value(0) - due to previous_letter changing from None to the first encountered letter.
...
with open(file_path, newline='') as csvfile:
    ...
    for row in reader:
        ...
        # Make image black and white (white = written text, black = background).
        # kernel_size = 3
        # blurred_image = cv2.GaussianBlur(new_image, (kernel_size, kernel_size), 0)
        # no "inv" required (cv2.TRESH_BINARY_INV) as the images are already with white written and black background.
